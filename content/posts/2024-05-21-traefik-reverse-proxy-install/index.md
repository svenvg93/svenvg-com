...
description: Install Traefik as a reverse proxy with Docker Compose or directly on the host via systemd, with automatic Let's Encrypt SSL through the Cloudflare DNS challenge.
date: 2024-05-21
draft: false
categories:
  - Networking
...
This guide covers two ways to run Traefik: with Docker Compose — simplest if the rest of your stack is already containerized — or directly on the host as a systemd service, with no Docker dependency and full control over the process. Both tracks below issue certificates automatically via Let's Encrypt using the Cloudflare DNS challenge.
...
Create a scoped API token in the Cloudflare dashboard with **Zone / Zone → Read** and **Zone / DNS → Edit** permissions. You'll store this token in an environment file — a `.env` file for Docker Compose, or a systemd environment file — covered below.
...
**Create the directory structure.** Set up the directories Traefik needs for config, dynamic routing rules, logs, and certificate storage. The `acme.json` file is where Let's Encrypt certificates are stored — it must be owner-readable only or Traefik will refuse to use it.
...
**Create a dedicated user.** Running Traefik as a dedicated non-root user limits what a compromised process can do. The `-r` flag creates a system account with no home directory, and `-s /sbin/nologin` prevents interactive login.
...
**Store the Cloudflare token** in an environment file that the systemd service will load:
...
**Main config.** This is the static config — it sets up entry points, the certificate resolver, and tells Traefik to watch `/etc/traefik/conf.d/` for dynamic service configs. Replace the `email` field with your own address for Let's Encrypt notifications.
